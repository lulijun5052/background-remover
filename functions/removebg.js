...
function parseMultipart(buffer, boundary) {
  ...
  while (true) {
    ...
    if (chunk[0] === 0x2D && chunk[1] === 0x2D) continue; // "--" end
    ...
    if (nm && fn) {
      parts.push({ name: nm[1], filename: fn[1], data: bodyData });
    }
  }
  return parts;
}
...
export async function onRequest(context) {
  try {
    ...
    if (!boundaryMatch) {
      return new Response(JSON.stringify({ error: 'Missing boundary', ct }), {
    ...
    if (!img) {
      return new Response(JSON.stringify({ 
        error: 'No image', 
        debug: { boundary, partsCount: parts.length, bodySize: body.byteLength }
      }), { headers: { 'Content-Type': 'application/json' } });
    }
